...
class MatrizDispersa():
	...
	#									que se haga siempre se devuelve el nodo con el nombre del dominio recibido
		...
		if self.cabeza is None:#No existe ninguna empresa, inserto la empresa en la cabeza, le asigno ese nodo a aux y lo retorno
			...
			while (temp is not None):#While utilizado para recorrer de forma horizontal las empresas
				nombresmes=[temp.valor, valor]
				nombresmes.sort()
				if temp.valor == valor:#si la empresa recibida ya existe me limito solo a igualarlo a aux para ser posteriormente retornado
					aux = temp
					bandera = 1
					print("Este mes ya existe ")
					break
				if (nombresmes.index(temp.valor)==1):#Si el nodo en el que estoy es mayor al que recibi significa que deberia
	...
	#									que se haga siempre se devuelve el nodo con el nombre del departamento recibido
		...
		if self.cabeza1 is None:#Si no existe ningun departamento lo agrego a la cabeza
			...
			while temp is not None:#utilizo este while para recorrer todos los nodos y validar algunos casos
				nombresfecha=[temp.valor, valor]
				nombresfecha.sort()
				if temp.valor == valor:#Si ya existe un nodo con el departamento recibido solo igualo el nodo a aux
					aux = temp
					bandera=1
					print("Este ano ya existe ")
					break
				if (nombresfecha.index(temp.valor)==1):#Si existe algun nodo con inicial mayor al departamento recibido significa que el departamento recibido
													#deberia ir arriba
	...
	def insertarValor(self, nodomes,valor,fecha,dia):#Este metodo recibe el nodo empresa,
		...
		if temp is None:#Si el nodo abajo del dominio es none se agrega y se iguala a aux
			...
			while temp is not None:#uso este while para recorrer los empleados que existen en la empresa recibida
				...
				#quiere decir que debo de enviarlo a atras
					...
					bandera = 1
					break#--------------------------
				if (tipofecha.index(temp.fecha)==1): #Averiguo si deberia meter mi nodo valores antes de cualquier otro
					if temp == nodomes:
						nodomes.abajo = nodo
						nodomes.abajo.arriba = nodomes
						aux = nodomes.abajo
	...
	def unir(self,nodofecha, NodoValores):#Este metodo lo utilizo para setear los apuntadores horizontales de mi nodo valores
	#									  por eso recibo como valores mi nodo letra y mi nodo valores(el cual ya tiene sus apuntadores verticales)
		if NodoValores != None:#Esta validacion la hago porque cuando agrego nodos atras de cualquier otro ya no retorno ese nodo porque ya tiene apuntadores
			bandera = 0
			print(nodofecha)
			if nodofecha.siguiente is None:#Si no existe ningun nodo siguiente al encabezado de la letra lo agrego ahi
				NodoValores.anterior = nodofecha
			...
			# nodo de valores porque su dominio es menor o si mi nodo valores va hasta de ultimo 
				...
				while  temp is not None:#Para recorrer los nodos valores apuntados por el nodo depto
					tipofecha=[temp.valor, NodoValores.fecha]
					tipofecha.sort()
					if(tipofecha.index(temp.fecha)==1):#si algun departamento es mayor al de mi nodo valores lo agrego antes
	...
	def getCodigoGraphviz(self):
		 print "digraph grafica{\n" + "rankdir=TB;\n" + "node [shape = record, style=filled, fillcolor=seashell2];\n" + self.graficarMatriz() + "}\n"
	 	 return "digraph grafica{\n" + "rankdir=TB;\n" + "node [shape = record, style=filled, fillcolor=seashell2];\n" + self.graficarMatriz() + "}\n"

	def graficarMatriz(self):
		texto =""
		tempfecha = self.cabeza1
		contador = 0
		while tempfecha != None:
			tempfecha.indice = contador
			texto += str(tempfecha.indice) +"[label =\"<C0>|" + tempfecha.valor + "|<C1>\"];\n"#imprime ano
			aux  = tempfecha.siguiente
			while aux!=None:
				contador+=1
				...
				aux = aux.siguiente
			contador += 1
			tempfecha = tempfecha.abajo
		tempfecha = self.cabeza1
		while tempfecha != None:
			if tempfecha.abajo != None:
				texto += str(tempfecha.indice) +"->"+str(tempfecha.abajo.indice)+";\n"
			if tempfecha != self.cabeza1:
				texto += str(tempfecha.indice) +"->"+str(tempfecha.arriba.indice)+";\n"
			aux  = tempfecha.siguiente
			while aux!=None:
...
#-----------------------------------------------------------
	def verificarExistencia(self,mes,fecha,dia):
...
if __name__ == "__main__":
	...
	encabezadomes = matriz.insertar("Diciembre")  # Envio mi empresa y recibo un nodo empresa
	encabezadofecha = matriz.insertar1("2016")  # Envio mi depto y recibo el nodo depto
	Nodo = matriz.insertarValor(encabezadomes, "1", "2016", "1")
	matriz.unir(encabezadofecha, Nodo)

	encabezadomes = matriz.insertar("Diciembre")  # Envio mi empresa y recibo un nodo empresa
	encabezadofecha = matriz.insertar1("2016")  # Envio mi depto y recibo el nodo depto
	Nodo = matriz.insertarValor(encabezadomes, "4", "2016", "4")
	matriz.unir(encabezadofecha, Nodo)

	encabezadomes = matriz.insertar("Marzo")  # Envio mi empresa y recibo un nodo empresa
	encabezadofecha = matriz.insertar1("2016")  # Envio mi depto y recibo el nodo depto
	Nodo = matriz.insertarValor(encabezadomes, "3", "2016", "3")
	matriz.unir(encabezadofecha, Nodo)

	encabezadomes = matriz.insertar("Marzo")  # Envio mi empresa y recibo un nodo empresa
	encabezadofecha = matriz.insertar1("2017")  # Envio mi depto y recibo el nodo depto
	Nodo = matriz.insertarValor(encabezadomes, "23", "2016", "23")
	matriz.unir(encabezadofecha, Nodo)
	matriz.visualizar1( encabezadomes)
